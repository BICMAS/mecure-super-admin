...
export const generateCourseTags = async (
  courseTitle: string,
  description: string,
): Promise<string[]> => {
...
export const analyzeSystemHealth = async (metrics: unknown): Promise<string> => {
...
export const suggestLearningPathDescription = async (
  pathTitle: string,
  courseList: string[],
): Promise<string> => {
  if (!apiKey) return "";
